...
class DEMData:
    ...
    def summary(self) -> Dict[str, Any]:
        return {
            ...
            "max_elve_m": self.max_elev,
        }
    
def _infer_hgt_grid_size(file_size_bytes: int) -> int:
    """Infer squarse HGT grid side from file size (2 bytes/sample)."""
...
def load_hgt(path: str | Path) -> DEMData:
    """
    Load STRM .hgt file (big-endian int16).
...
def merge_dems(dem_list: list[DEMData], source_path: str | Path) -> DEMData:
    ...
    if not dem_list:
        raise ValueError("DEM list is empty. Please provide at least one DEMData.")
...
def load_dem(path: str | Path) -> DEMData:
    """Load DEM automatically based on extension (.hgt)."""
    p = Path(path)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Load DEM (.hgt) and print metadata summary.")
